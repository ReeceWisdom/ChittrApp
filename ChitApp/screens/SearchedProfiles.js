...
class SearchedProfilesPage extends Component {
  ...
  constructor(props) {
    ...
    this.state = {
      ...
      family_name: '',
      email: '',
      q: this.props.navigation.state.params.user_id,
      searchedList: []
    }
  }
  ...
  getData(q) {
    let request = "http:10.0.2.2:3333/api/v0.0.5/search_user/?q=" + q;
    console.log("Searched Profiles Request:", request);
    ...
    .then((responseJson) => {
      ...
      console.log("JSON Request Results:", this.state.searchedList);
    })
    .catch((error) => {
      console.log(error);
    });
    ...
    return (
      <View style={{ flex: 1, flexDirection: 'column', marginBottom: 3 }}>
        <View style={{ backgroundColor: '#1DCAFF', alignItems: 'center', justifyContent: 'center', borderWidth: 5, borderColor: '#000000', borderLeftWidth: 1, borderRightWidth: 1 }}>
          <Text style={{ color: '#000000', fontSize: 24, fontWeight: 'bold', height: 50, paddingTop: 8 }}>~~~ Searched Users ~~~</Text>
        </View>
...
export default SearchedProfilesPage;